		...
		bool *force_flush, bool *force_break, bool *any_skipped)
{
	...
	if (!delay_rmap) {
		 ...
		 * present in this table.  Large folios keep the exact per-PTE path.
		 */
		if (!folio_test_large(folio))
			pgcl143_floor_remove(folio, page, vma, pte, addr,
					     pte_pfn(ptent));
		else
	...
	if (folio_test_large(folio) ||
		...
		if (nr > 1 || folio_test_large(folio)) {
			...
			if (folio_test_large(folio)) {
				 ...
				 * remove each sub-PTE's _mapcount edge, but never drive the
				 * cluster's _mapcount below the sub-PTEs still present in
				 * this table (present_here).  The spurious/over-remove that
				 * undercounts and defeats the folio_mapped() free-while-
...
check_folio:
	...
	/*
	 * PGCL: one struct page backs PAGE_MMUCOUNT sub-PTEs that share the
	 * cluster's swap slot and its single PG_anon_exclusive bit.  Fragments
	 * swap in one at a time, so the first to map makes the folio anon and
	 * sets head-exclusive (folio_add_new_anon_rmap), and later siblings
	 * legitimately find it already set.  That is not the violation this
	 * guards -- a *shared* (non-exclusive) PTE pointing at an exclusive page.
	 * Fire only when the faulting swap PTE is itself NOT exclusive while the
