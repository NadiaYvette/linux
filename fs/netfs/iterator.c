				...
				iov_iter_extraction_t extraction_flags)
{
	...
	while (count && npages < max_pages) {
		...
		count -= ret;
		/*
		 * GUP returns one entry per MMUPAGE.  Compute the MMUPAGE
		 * count from the data bytes and the sub-MMUPAGE offset.
		 */
		cur_npages = DIV_ROUND_UP(ret + (offset & ~MMUPAGE_MASK),
					  MMUPAGE_SIZE);
